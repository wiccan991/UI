...
namespace QCYDS9_HFT_2023241.Client
{
    internal class Program
    {
        ...
        static AstronautLogic astrol;
        static MissionLogic missl;
        static SpacehsipLogic spacel;

        static void List(string entity) 
        {
            if (entity == "Astronaut")
            {
                var items = astrol.ReadAll();
                Console.WriteLine("id" + "\t" + "name");
                foreach (var item in items) { Console.WriteLine(item.AstronautId + "\t" + item.Name); }
            }
            else if (entity == "Mission")
            {
                var items = missl.ReadAll();
                Console.WriteLine("id" + "\t" + "name");
                foreach (var item in items) { Console.WriteLine(item.MissionId + "\t" + item.Name); }
            }
            else if (entity == "Spaceship")
            {
                var items = spacel.ReadAll();
                Console.WriteLine("id" + "\t" + "name");
                foreach (var item in items) { Console.WriteLine(item.Id + "\t" + item.Name); }
            }
        }
        ...
        {
            ////rest = new RestService("http://localhost:63809/api/", "spaceship");
            var spaceshipSub = new ConsoleMenu(args, level: 1)
                ...
                //.Add("Cheap Squad Info", () => YouthSquadInfo("Spaceship"))
